...
*企业付款接口
*/
class WxMchPay_pub extends Wxpay_client_pub
{
    ...
    function createXml()
    {
        try
        {
            ...
            if($this->parameters["openid"] == null ) {
                throw new SDKRuntimeException("企业付款申请接口中，openid至少填一个！"."<br>");
            }elseif($this->parameters["amount"] == null){
                throw new SDKRuntimeException("企业付款申请接口中，缺少必填参数付款金额amount！"."<br>");
            }elseif($this->parameters["desc"] == null){
                throw new SDKRuntimeException("企业付款申请接口中，缺少必填参数付款信息描述desc！"."<br>");
            }
            $this->parameters["mch_appid"] = WxPayConf_pub::APPID;//公众账号ID
            $this->parameters["mchid"] = WxPayConf_pub::MCHID;//商户号
            $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
            $this->parameters['partner_trade_no'] = WxPayConf_pub::MCHID.time();// 商户订单号
            $this->parameters['check_name'] = NO_CHECK;// 校验用户姓名选项为不校验
            $this->parameters['spbill_create_ip'] = $_SERVER['REMOTE_ADDR'];//IP地址
            $this->parameters["sign"] = $this->getSign($this->parameters);//生成签名，注意签名必须在最后
            return $this->arrayToXml($this->parameters);
...
*企业发红包接口
*/
class Wxredpack_pub extends Wxpay_client_pub
{
    ...
    function createXml()
    {
        try
        {
            ...
            if($this->parameters["re_openid"] == null ) {
                throw new SDKRuntimeException("红包接口中，缺少必填参数re_openid！"."<br>");
            }elseif($this->parameters["total_amount"] == null){
                throw new SDKRuntimeException("红包接口中，缺少必填参数付款金额total_amount！"."<br>");
            }elseif($this->parameters["wishing"] == null){
                throw new SDKRuntimeException("红包接口中，缺少必填参数红包祝福语wishing！"."<br>");
            }elseif($this->parameters["act_name"] == null){
                throw new SDKRuntimeException("红包接口中，缺少必填参数红包活动名称act_name！"."<br>");
            }elseif($this->parameters["remark"] == null){
                throw new SDKRuntimeException("红包接口中，缺少必填参数红包备注信息remark！"."<br>");
            }
            $this->parameters["wxappid"] = WxPayConf_pub::APPID;//公众账号ID
            $this->parameters["mch_id"] = WxPayConf_pub::MCHID;//商户号
            $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
            $this->parameters['mch_billno'] = WxPayConf_pub::MCHID.date("Ymd").time();// 商户订单号
            $this->parameters['send_name'] = WxPayConf_pub::SENDNAME;//商户名称红包发送者名称
            $this->parameters['total_num'] = 1;//红包发放总人数 
            $this->parameters['client_ip'] = $_SERVER['REMOTE_ADDR'];//IP地址
            $this->parameters["sign"] = $this->getSign($this->parameters);//生成签名，注意签名必须在最后
            return $this->arrayToXml($this->parameters);
