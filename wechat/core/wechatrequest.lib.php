...
class WechatRequest{
    ...
    public static function location(&$request){
        $Longitude = $request['location_y'];
        $Latitude = $request['location_x'];
        $content = '收到上报的地理位置X:'.$Longitude.',Y:'.$Latitude;
        return ResponsePassive::text($request['fromusername'], $request['tousername'], $content);
    }
    ...
    public static function eventQrsceneSubscribe(&$request){
        $openid = $request['fromusername'];//取得新用户的openid
        $sceneid = $request['eventkey'];//取得带参数二维码场景值
        $affected = Scanin::dealwithTable($sceneid,$openid);//调用scanin对象表处理方法将关联关系写入no+场景值表名中,成功放回一个int数1，表示插入1条记录成功。错误放回string在$affected[2]中

        $content = '欢迎您关注我们的微信，将为您竭诚服务';
        return ResponsePassive::text($request['fromusername'], $request['tousername'], $content);//测试时可用$affected[2]替换$content,可在微信客户端输出PDO错误代码信息
    }
    ...
    public static function eventLocation(&$request){
        ...
        $content = '收到上报的地理信息X:'.$Longitude.',Y:'.$Latitude;
        return ResponsePassive::text($request['fromusername'], $request['tousername'], $content);
    }
    ...
    public static function eventClick(&$request){
		//获取该分类的信息
        //$content = $request['eventkey'];
       
        $openid = $request['fromusername'];
            switch ($request['eventkey']) {//用eventkey来分别触发菜单点击事件
                case 'key_1'://分享二维码菜单点击事件
                   //$url = Promo::createQrcode($openid); 
                   $content = "'点击链接'.$url.'展示您的专享二维码,分享赢红包!'";
                   return ResponsePassive::text($request['fromusername'], $request['tousername'], $content);
